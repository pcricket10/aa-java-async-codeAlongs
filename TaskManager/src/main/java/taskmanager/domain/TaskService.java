...
public class TaskService {
    ...
    public TaskResult update(Task task) throws DataAccessException {
        ...
        boolean updated = repository.update(task); // we are trying to update - if this fails it's because the id we are trying to update doesn't exist
        if(!updated){
    ...
    /*
    we cannot add a task with an id > 0
    we need to check that createdOn exists
    we need to check if a title exists and it cannot be greater than 50 char
    we need to make sure that there is a description and the description cannot be less than 20 characters
    we need to make sure there is a dueDate
    we need to ensure the task has a status
